...
// ⚑ VERSION BY CONTENT, AND NORMALISE THE LINE ENDINGS FIRST. Stamping the cache with a byte count
// made the number differ between a Windows checkout and the Linux runner — so the "is the page
// stale" check could never pass, and a real drift check that always fails is one people delete.
const stamp = createHash('sha256').update(out.split('\r\n').join('\n')).digest('hex').slice(0, 12);
